...
export interface PlaygroundLayoutProps {
  ...
  isDesktop: boolean;
  rightPanelRef: MutableRefObject<HTMLDivElement | null>;
  collapsePanel: () => void;
  expandPanel: () => void;
...
export function PlaygroundLayout({
  ...
  const _COLLAPSED_MOBILE_HEIGHT = COLLAPSED_MOBILE_HEIGHT;
  const _COLLAPSED_DESKTOP_WIDTH = COLLAPSED_DESKTOP_WIDTH;
  ...
  useEffect(() => {
    ...
    const leftRef = rightPanelRef.current;
    ...
    const mouseDownHandler = (e: MouseEvent | TouchEvent) => {
      ...
      isDesktop
        ? (leftWidth = rightPanelRef.current?.getBoundingClientRect().width!)
        : (topHeight = rightPanelRef.current?.getBoundingClientRect().height!);
    ...
    const handleResizerDoubleClick = () => {
      ...
      if (!rightPanelRef.current || !rightSide.current) return;

      const currentSize = isDesktop
        ? parseInt(getComputedStyle(rightPanelRef.current).width)
        : parseInt(getComputedStyle(rightPanelRef.current).height);
