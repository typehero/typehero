...
import type { UserPrivateProfile } from './profile.actions';
import {
  Card,
  CardContent,
  CardDescription,
...
} from '~/app/[locale]/explore/_components/explore-card';
    ...
    <div className="flex flex-col flex-wrap items-center gap-4 md:flex-row md:items-start">
      {privateUser.bookmark.map(({ challenge }, idx) => {
        ...
        return (
          <Link key={`bookmark-challenge-${idx}`} href={`/challenge/${challenge.id}`}>
            <Card
              className={`group/card bg-background hover:bg-card-hovered relative w-[300px] overflow-hidden duration-300
            ...
            >
              ...
              <CardContent className="relative flex flex-col justify-between gap-2 rounded-xl p-6 pb-0 pt-0 duration-300">
                ...
                <CardDescription className="relative h-20 pb-4">
                  <div className="pointer-events-none absolute inset-0 h-full w-full text-sm shadow-[inset_0_-1.5rem_1rem_-0.5rem_hsl(var(--card))] duration-300 group-hover/card:shadow-[inset_0_-1.5rem_1rem_-0.5rem_hsl(var(--card-hovered))] group-focus:shadow-[inset_0_-1.5rem_1rem_-0.5rem_hsl(var(--card-hovered))]" />
      ...
      {privateUser.bookmark.length === 0 ? (
        <div className="flex w-full flex-col items-center justify-center gap-2">
          <span className="text-base font-semibold md:text-lg">type NoBookmarks</span>
          <span className="text-muted-foreground text-center text-sm">
            We couldn&apos;t find any bookmarks. Try adding one!
          </span>
        </div>
      ) : null}
    </div>
