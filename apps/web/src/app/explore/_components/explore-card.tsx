...
export function ExploreCard({ challenge }: ExploreCardProps) {
  return (
    <Card
      className={`group/card relative overflow-hidden duration-300 sm:min-w-[300px] xl:min-w-[333px]
    ...
    >
      {challenge.difficulty === 'BEGINNER' && (
        <>
          <Circle className="absolute -right-4 -top-8 h-24 w-24 origin-top-right stroke-1 text-white/30 duration-300 group-hover/card:scale-90 dark:group-hover/card:text-black/30" />
          <Circle className="absolute -right-4 -top-8 h-32 w-32 origin-top-right stroke-1 text-white/30 duration-500 group-hover/card:scale-90 dark:group-hover/card:text-black/30" />
        </>
      )}
      {challenge.difficulty === 'EASY' && (
        <>
          <Diamond className="absolute -right-5 -top-10 h-24 w-24 origin-top-right stroke-1 text-white/30 duration-300 group-hover/card:rotate-6 group-hover/card:scale-90 dark:group-hover/card:text-black/30" />
          <Diamond className="absolute -right-6 -top-12 h-36 w-36 rotate-12 stroke-[0.75] text-white/30 duration-300 group-hover/card:-translate-y-2 group-hover/card:translate-x-3 group-hover/card:rotate-6 group-hover/card:scale-90 dark:group-hover/card:text-black/30" />
        </>
      )}
      {challenge.difficulty === 'MEDIUM' && (
        <>
          <Triangle className="absolute -right-5 -top-5 h-16 w-16 rotate-0 stroke-2 text-white/50 duration-500 group-hover/card:-translate-x-10 group-hover/card:translate-y-10 group-hover/card:rotate-[90deg] dark:group-hover/card:text-black/30" />
          <Triangle className="absolute -right-14 -top-16 h-36 w-36 rotate-0 stroke-1 text-white/50 duration-300 group-hover/card:translate-x-3 group-hover/card:rotate-[30deg] group-hover/card:scale-50 group-hover/card:stroke-2 dark:group-hover/card:text-black/30" />
        </>
      )}
      {challenge.difficulty === 'HARD' && (
        <>
          <Plus className="absolute -right-4 -top-8 h-24 w-24 stroke-1 text-white/30 duration-300 group-hover/card:scale-0 dark:group-hover/card:text-black/30" />
          <Plus className="absolute -right-4 -top-8 h-32 w-32 stroke-1 text-white/30 duration-500 group-hover/card:-translate-y-5 group-hover/card:translate-x-5 group-hover/card:-rotate-90 group-hover/card:scale-75 dark:group-hover/card:text-black/30" />
        </>
      )}
      {challenge.difficulty === 'EXTREME' && (
        <>
          <Sparkle className="absolute -right-4 -top-10 h-24 w-24 stroke-1 text-white/40 duration-500 group-hover/card:-translate-x-4 group-hover/card:translate-y-10 group-hover/card:-rotate-[125deg] dark:group-hover/card:text-black/30" />
          <Sparkle className="absolute -right-12 -top-20 h-48 w-48 origin-top-right stroke-1 text-white/40 duration-300 group-hover/card:scale-50 dark:group-hover/card:text-black/30" />
        </>
      )}
      <CardHeader className="relative flex flex-col items-start gap-1 py-5">
        <CardTitle className="max-w-[75%] truncate text-2xl text-white duration-300 dark:text-white dark:group-hover/card:text-black/70">
          {challenge.name}
        </CardTitle>
        <div className="flex items-center gap-6 text-center text-white duration-300 dark:group-hover/card:text-black">
      ...
      <CardContent className="bg-background group-hover/card:bg-card-hovered relative flex flex-col justify-between gap-2 rounded-xl p-6 pb-0 duration-300">
