...
export function LoginLink({ className }: { className?: string }) {
  ...
  return (
    <Link
      className={clsx(
        'w-20 rounded-md border border-transparent bg-transparent p-2 text-black duration-300 hover:border-black/30 hover:bg-black/10 focus:bg-transparent focus:outline-none dark:text-white dark:hover:border-white/30 dark:hover:bg-white/10',
        className,
      )}
      href={{
        pathname: '/login',
