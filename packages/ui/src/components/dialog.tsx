...
>(({ className, children, displayX = true, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      {displayX ? (
        <DialogPrimitive.Close className="ring-offset-background focus:ring-ring data-[state=open]:bg-accent data-[state=open]:text-muted-foreground absolute right-4 top-4 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-offset-0 disabled:pointer-events-none">
          <X className="h-4 w-4" />
