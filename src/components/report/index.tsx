...
import { type ChallengeReport, type UserReport, type CommentReport, addReport, type SolutionReport } from '~/components/report/report.action';
...
export default function ReportDialog({ children, triggerAsChild = false, reportType = 'COMMENT', ...props }: React.PropsWithChildren<ReportUserDialogProps | ReportChallengeDialogProps | ReportCommentDialogProps | ReportSolutionDialogProps>) {
  ...
  return (
    <Dialog open={show} onOpenChange={e => {
      setShow(e);
    }}>
          ...
          switch(value) {
            case 'already_exists': 
              toast({
                ...
                description: (
                  <p>You have already made a report with this information.</p>
                )
              })
              break;
            case 'created':
            ...
            case 'not_logged_in':
              toast({
                ...
                variant: 'destructive',
                description: (
                  <p>You must be logged in to make a report.</p>
                )
              })
          }
          setShow(false);
              ...
              render={({ field }) => (
                <FormItem className='my-3'>
                  <div className="flex flex-col gap-3">
                    ...
                    <Textarea {...register('comments')} placeholder="Any extra information to help our moderator team when reviewing this report." />
                  </div>
                </FormItem>
              )}
              control={control}
